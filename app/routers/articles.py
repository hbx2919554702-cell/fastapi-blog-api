...
# 创建文章
@router.post("/create", response_model=ArticleCreate)
def create_article(article:ArticleCreate,db:Session = Depends(get_db)):
    return articles.create_article(db=db,article=article)

# 根据id搜索文章
@router.get("/get_articles_id/{articles_id}", response_model=ArticleCreate)
...
# 模糊搜索
@router.get("/get_articles", response_model=List[ArticleCreate])
